...
|-----:|:----------------------|:-----------------------------------------------------------------|
| 1    | OT                    | 프로젝트 계획서 작성                                             |
| 2    | 프로젝트 계획서 리뷰   | PoC(Baseline vs Mixup 실험, Hyperparameter Tuning)                    |
| 3    | 주간 보고서 제출      | PoC(Pose + LSTM 파이프라인 구축)                              |
| 4    | 제안서 발표(1학기)    | Rust-Python 연동 기본 PoC 설계, Rust-Python 연동 PoC 구현 |
| 5    | 주간 보고서 제출      | 데이터 확보 및 라벨링, LSTM 모델 재학습 및 파인 튜닝, 와이퍼프레임 설계 및 프로토타입 구현 |
| 6    | 주간 보고서 제출      | 차량 접근 이벤트 로직 개발, Edge Device 초기 세팅, UI 피드백 수집 및 개선 |
| 7    | 주간 보고서 제출      | -                                                               |
| 8    | -                 | TensorRT/ONNX 최적화 작업, Rust 통합 설계 구체화, 발표 자료 준비      |
| 9    | 중간 발표(1학기)      | 실시간 시퀀스 처리 최적화, 데이터 증강 및 도메인 어댑테이션, 오탐/미탐 사례 분석 |
...
![825E8621-2F71-44AB-B3EA-1CABF5826850](https://github.com/user-attachments/assets/8c1b7d7c-dfb9-4462-bb89-ede6c8203206)

## 1. Rust-Python PoC 작업

### 목적
- Rust 기반 시스템과 Python으로 작성된 추론 코드(LSTM, YOLO 등)를 연동하여, 양쪽의 통신 및 데이터 전달이 원활하게 이루어지는지 검증함.

### 구현 내용
- **Python 측 (`python_inference.py`):**
  - 더미 함수 `inference_dummy(x)`를 정의하여, 입력값의 2배를 반환하도록 구현.
  - 향후 LSTM 모델 추론 코드로 쉽게 교체할 수 있도록 기본 구조 마련.

- **Rust 측 (`src/main.rs`):**
  - [PyO3](https://pyo3.rs/) 라이브러리를 활용하여 Python 인터프리터를 초기화.
  - `python_inference` 모듈을 임포트 후, `inference_dummy` 함수를 호출하여 결과를 받아 출력함.

### 결과 및 의의
- Rust에서 Python 함수를 성공적으로 호출하고, 결과를 올바르게 처리하는 구조를 확인함.
- 이 PoC는 향후 Edge 디바이스에서 실제 딥러닝 모델(예: Pose, LSTM 등)을 연동할 때 중요한 기반이 될 것임.

---

## 2. 데이터셋 확보

### 2.1 정상인 데이터 (약 2시간 분량)
- **데이터 출처:**  
  - **Caltech Pedestrian Dataset**
    - 보행자 검출 및 행동 분석에 널리 사용되는 공개 데이터셋.
- **활용 계획:**  
  - 정상인의 보행 및 다양한 행동 패턴을 학습 데이터로 활용하여, LSTM 등 시계열 분석 모델의 정상 상태(비주취자) 기준을 마련.
  - 데이터 다양성이 부족할 경우, 증강 기법을 추가 적용할 예정.

### 2.2 주취자 데이터 (약 2시간 분량)
- **데이터 출처:**  
  - 본인이 직접 촬영한 영상  
    - 연출을 통해 만취 상태(몸 흔들림, 자세 기울기 등)를 명확하게 표현한 장면을 확보.
- **활용 계획:**  
  - 주취자의 특징(불안정한 보행, 비정상적 자세 등)을 잘 반영하는 데이터를 확보하여, LSTM 모델 학습 시 정상인과 주취자 행동의 차이를 명확하게 구분할 수 있도록 함.

---

## 3. 시스템 아키텍처

프로젝트 전체의 데이터 흐름과 시스템 통합을 아래 Mermaid 다이어그램으로 설계하였습니다.

- **데이터 수집:**
  - CCTV 또는 Edge Device에서 비주취자 영상을, 본인이 직접 촬영한 주취자 영상을 수집합니다.
  
- **데이터 전처리 및 라벨링:**
  - 수집한 영상을 전처리 및 증강하고, Caltech Pedestrian Dataset과 직접 촬영한 데이터를 기반으로 라벨링합니다.
  
- **모델 학습 및 추론:**
  - LSTM 및 YOLO 모델을 학습시키고, Python 기반 추론 모듈을 통해 결과를 도출합니다.
  
- **시스템 통합:**
  - Rust Backend가 Python 추론 결과를 받아 처리하고, UI 대시보드를 통해 최종 결과를 시각화합니다.
...
- **Rust-Python 연동 PoC:**  
  - Rust와 Python 간의 통신이 원활하게 이루어지는 것을 확인함.
  - 향후 실제 LSTM 및 기타 딥러닝 모델 연동 시 이 구조를 확장하여 적용할 예정임.

- **데이터셋 확보:**  
  - 정상인 데이터는 Caltech Pedestrian Dataset을 기본으로 사용하며, 주취자 데이터는 본인 촬영 2시간 분량을 확보.
  - 총 4시간의 원본 데이터를 기반으로, 증강 및 합성을 통해 데이터 다양성을 보완할 계획임.

- **향후 작업:**  
  - 5주차부터 확보한 데이터셋을 기반으로 LSTM 모델 재학습 및 성능 최적화 작업 진행.
  - 모델 학습 후, 실시간 추론 및 시스템 통합 테스트를 통해 전체 파이프라인을 개선할 예정.
...
### 1.1 데이터 준비
- **정상인(비주취자):** Caltech Pedestrian + 일부 공개 CCTV 영상
- **주취자:** 본인 촬영 2시간 (주간, 야간, 주차장 등 다양)
- **전처리:** 30초~1분 단위로 분할, 시퀀스 형태(Pose Keypoint 등) 생성  
- **증강:** Data imbalance 보완을 위해 flip, random noise 적용

### 1.2 학습 과정
- **모델 구조:** 기존 LSTM(1~2 레이어, hidden size=128, dropout=0.3)
- **학습 Epoch:** 10~20 사이에서 Early Stopping 적용
- **성과:**  
  - Accuracy, F1 등 5~7%p 상승(4주차 대비)  
  - 일부 야간·우천 장면에서 Recall 낮아짐

---

## 2. Rust-Python 연동 PoC 구현 (확장)

### 2.1 구현 개요
- **PyO3**를 활용해 Rust에서 Python 모듈을 임포트, 추론 함수 호출
- 기존 **inference_dummy** 함수를 **실제 LSTM 추론 함수**로 확장 준비

### 2.2 데이터 전달 방식
- **파일 기반**: Rust가 영상/프레임 파일을 Python에 넘겨 추론
- **성능 측정**:  
  - 데스크톱 환경에서 10~15 FPS  
  - Edge Device 최적화는 차후 진행
...
### 3.1 주취자 vs 비주취자 분류
- **정확도**: 약 80% 이상  
- **오탐/미탐 사례**: 야간·주차장 등 어두운 환경에서 주취자 검출이 불안정

### 3.2 Rust-Python 성능
- **처리 지연**: 200~300ms (데스크톱 기준)
- **개선 여지**: ONNXRuntime, TensorRT 등으로 추후 최적화
...
### 4.1 개발 문서
- LSTM 모델 구조, Rust-Python 연동 구조 정리
- 데이터 파이프라인 및 전처리 흐름 문서화

### 4.2 결과 보고
- **Confusion Matrix**, **Precision/Recall** 그래프 등 첨부
- PoC 성능(FPS, Latency) 측정치 기록

### 4.3 이슈 및 개선
- 야간 환경 데이터 부족 → 추가 촬영 또는 증강  
- PyO3 연동 시 메모리 전달 방식 도입 검토
...
- **5주차 결론:**
  - LSTM 재학습을 통해 정확도가 일부 상승했으며, Rust-Python 연동 PoC를 기반으로 실제 추론에 근접한 구조를 마련.
  - 어두운 환경에서 발생하는 주취자 분류 문제를 해결하기 위해 추가 데이터 확보 또는 증강이 필요.

- **향후 작업:**
  1. 데이터 증강/추가 촬영 (야간·우천 상황 강화)
  2. Python 추론 코드 최적화 (ONNX/TensorRT) 및 Rust 연동 고도화
  3. UI 대시보드 연동: Rust Backend + 웹 인터페이스 구체화
  4. Edge Device 실험 및 실시간 추론 성능 측정
