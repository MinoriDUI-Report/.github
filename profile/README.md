...
3월 10일부터 3월 11일까지 이틀 동안 진행한 PoC 단계에서는 영상 속 객체들 중 사람, 자동차, 오토바이, 자동차(문 열린 상태, **음주 운전 가능성 있음**), 자동차(문 닫힌 상태)로 분류할 수 있는 ML 모델을 제작하였습니다.

1. **데이터셋 활용 및 초기 테스트 (Google Colab)**
   ...
   - **Google Colab 환경**:  
     - 로컬 CPU 환경에서 학습 속도 문제(수 시간~수십 시간 예상)로 인해, Google Colab의 GPU(예: Tesla T4)로 전환하여 PoC를 진행하였습니다.  
     - Epoch 1~10 수준의 **간단한 Fine-tuning**으로도, mAP(초기값) ??%대 내외의 결과를 확인하였으나, 더 높은 정확도를 위해 추가 학습이 필요함을 인지하였습니다.
    
2. 2. **학습 로그 및 성능 측정**  
   - **학습 로그 예시**:  
     - Epoch 1~3 사이 box_loss, cls_loss, dfl_loss 등이 지속 감소하는 양상을 보였으며, 전반적으로 정상 학습 흐름이 관찰됨.  
     - Colab 환경에서 Batch size=16, 해상도 640×640 기준, 1 Epoch에 약 1시간 소요 (초기 실험).  
   - **객체 감지 정확도(mAP)**:  
     - 초기(5 Epoch 이내)에는 ??대 mAP를 얻었으나, 추가 Epoch(10 이상)로 진행 시 ??%대까지 개선 가능성을 확인.  
   - **추가 개선 방안**:  
