...
### A) 대한민국 각 지역 CCTV 실시간 선별 관제 시스템 (오산시)  
- **사례 설명:**  
  오산시에서는 기존 CCTV를 활용하여, 범죄 의심 정도를 실시간으로 선별하고 관제하는 시스템을 도입하였습니다.  
- **기술적 접근:**  
...
- **사례 설명:**  
  영국의 Acusensus AI 카메라는 운전 중 휴대폰 사용, 약물 복용, 음주 운전 등 운전자의 부적절한 행동을 실시간으로 감지하는 데 중점을 두고 있습니다.  
- **기술적 접근:**  
  - 고해상도 영상과 AI 기반 행동 분석 알고리즘을 결합하여, 운전 중 위험 행위를 정확하게 식별합니다.  
  - 실시간 경고 시스템을 통해 해당 운전자에게 즉각적인 경고를 전달하고, 관제 센터로 데이터를 전송합니다.
- **특장점 및 한계:**  
  - **장점:** 운전자 행동에 대한 세밀한 분석으로, 음주 및 약물 복용과 같은 특정 부적절 행위를 효과적으로 감지합니다.  
...
#### C) 호주 에디스코완대(ECU) 연구팀의 AI 기반 차량 내 음주운전 감지 기술  
- **사례 설명:**  
...
우리 시스템은 기존 경쟁 모델과 비교하여 다음과 같은 주요 강점을 보유하고 있습니다:
...
이와 같이, 우리 시스템은 Rust 기반 Edge Computing 최적화를 통해 경쟁 모델 대비 뛰어난 성능과 비용 효율성을 제공함으로써, 음주운전 예방 및 국민 안전 강화에 크게 기여할 수 있습니다.

---

## 3. 구현

본 장에서는 초기 PoC를 통해 얻은 인사이트와, 이를 바탕으로 한 전체 시스템 구현 계획을 구분하여 제시합니다.

### 3.1 PoC 수행 결과 및 인사이트

3월 10일부터 3월 11일까지 이틀동안 진행한 PoC 단계에서는 다음 사항들을 검증하였습니다:

- **데이터셋 활용 및 초기 테스트:**
  - **기본 데이터셋 "car_driver_seat":**
    - Roboflow Universe (v13, CC BY 4.0)에서 약 7,096장의 데이터를 활용하여, `car`, `car_door_opened`, `motocycle`, `open_driver_seat`, `person` 등의 클래스로 모델 학습을 진행했습니다.  
    - 데이터의 해상도(1024×1024), 회전(±15°), 밝기 증강(±25%) 등이 실제 PoC 테스트에서 모델의 초기 성능을 확보하는 데 기여했습니다.

- **개인정보 보호 테스트:**  
  - PoC에서는 Gaussian Blur와 박스형 마스킹 등 기본적인 익명화 기술을 적용하여, 얼굴 및 번호판 보호 효과를 검증하였습니다.
  - CC BY 4.0 라이선스 준수와 관련 문서화 절차를 점검하였으며, 간단한 익명화 파이프라인을 구성하여 실제 CCTV 영상 데이터에 적용해 보았습니다.

- **초기 알고리즘 성능 및 구현:**  
  - YOLOv8 기반의 초기 모델 파인튜닝을 진행하였으며, `intoxicated_person` 라벨 도입 가능성을 실험했습니다.  
  - PoC를 통해 모델의 기본 추론 성능과 일부 경고 기능(예: 단순 경고 화면 전환)이 구현되었음을 확인하였습니다.
  - 간단한 웹 대시보드 프로토타입을 통해, 실시간 영상 스트리밍과 이벤트 로그의 기본 UI/UX를 점검했습니다.

*PoC 결과를 통해, 해당 데이터셋 및 초기 모델의 활용 가능성을 확인하였습니다.*
...
| 3    | 초기 UI/UX 와이어프레임, 기계학습 환경 설정, YOLOv8 기반 모델 Fine-tuning 시작, 초기 모델 학습 로그, 체크포인트 생성
| 4    | 데이터 증강 기법 적용 및 효과 분석(하이퍼파라미터 조정 실험)
| 5    | Cross Validation을 통한 모델 성능 검증
