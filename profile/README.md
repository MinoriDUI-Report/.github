...
3월 10일부터 3월 11일까지 이틀 동안 진행한 PoC 단계에서는 다음 사항들을 검증하였습니다:

- **데이터셋 활용 및 초기 테스트:**  
  - **기본 데이터셋 "car_driver_seat":**  
    - Roboflow Universe (v13, CC BY 4.0)에서 약 7,096장의 데이터를 사용하여, `car`, `car_door_opened`, `motocycle`, `open_driver_seat`, `person` 등의 클래스로 모델 학습을 진행했습니다.  
    - 데이터셋에 기본으로 적용된 해상도(1024×1024), 회전(±15°), 밝기 증강(±25%) 등 기본 설정을 그대로 활용하여 초기 성능을 확인했습니다.

- **초기 알고리즘 성능 및 구현:**  
  - YOLOv8 기반의 초기 모델 파인튜닝을 진행 중에 있으며, `intoxicated_person` 라벨 도입 가능성에 대해 일부 실험을 진행했습니다.  
  - 아직 파인튜닝이 완전히 마무리되지 않았으므로, 초기 테스트 결과는 추가 개선 및 재검증이 필요합니다.

- **실행 환경 전환:**  
  - 로컬 환경에서 테스트를 시도했으나 속도 문제가 발생하여, Google Colab 환경으로 전환하여 효율적으로 PoC를 진행하였습니다.
